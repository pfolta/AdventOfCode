...
private data class CrabCombatGame(
    ...
    fun computeScore() = (player1 + player2).reversed().mapIndexed { position, card -> card * (position + 1) }.sum()
        ...
        while (player1.isNotEmpty() && player2.isNotEmpty()) {
            ...
            val winningPlayer = when {
                player1.size >= card1 && player2.size >= card2 ->
                    CrabCombatGame(player1.take(card1).toMutableList(), player2.take(card2).toMutableList()).playRecursiveGame(false)
                card1 > card2 -> 1
                else -> 2
            }
